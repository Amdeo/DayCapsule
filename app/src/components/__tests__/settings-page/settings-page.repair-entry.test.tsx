...
describe('SettingsPage repair entry', () => {
  ...
  it('does not render the E2E sync lab section when the env flag is disabled', async () => {
    const { screen } = await renderSettingsPage({ e2eSyncLab: false });
  ...
  it('keeps the dedicated repair entry available after injecting the suspect + repairable fixture', async () => {
    const { screen, mocks } = await renderSettingsPage({ e2eSyncLab: true });
